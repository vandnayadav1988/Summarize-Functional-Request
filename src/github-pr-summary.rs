...
async fn handler(
    ...
    for (_i, commit) in commits.iter().enumerate() {
        ...
        let question = "The following is a GitHub patch. Please ensure that the code is aligned with the respective files in swagger directory?".to_string() + truncate(commit, CHAR_SOFT_LIMIT);
    ...
    if reviews.len() > 1 {
        ...
        let question = "Here is a set of summaries for functional changes. Each summary starts with a ------ line. Please write an comprehensive summary to ensure that the code is aligned with the swagger files. Please first present the most important findings, in your summary.".to_string() + &reviews_text;
        match openai.chat_completion(&chat_id, &question, &co).await {
            Ok(r) => {
