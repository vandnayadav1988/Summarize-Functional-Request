...
async fn handler(
    ...
    for (_i, commit) in commits.iter().enumerate() {
        ...
        let question = "The following is a GitHub patch. compare the cod with swagger/forms/post_identities.yaml file ".to_string() + truncate(commit, CHAR_SOFT_LIMIT);
    ...
    if reviews.len() > 1 {
        ...
        let question = "Here is a set of summaries for functional changes. Each summary starts with a ------ line. Please compare the code with swagger/forms/post_identities.yaml . Please first present the most important findings, in your summary.".to_string() + &reviews_text;
        match openai.chat_completion(&chat_id, &question, &co).await {
            Ok(r) => {
